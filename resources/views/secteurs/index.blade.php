...
<div class="max-w-7xl mx-auto space-y-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-slate-900">🏢 Gestion des Secteurs</h1>
        ...
        <table class="min-w-full divide-y divide-slate-200">
            <thead class="bg-slate-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Nom du
                        Secteur</th>
            ...
            <tbody class="bg-white divide-y divide-slate-200">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">#{{ $secteur->id_secteur }}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-slate-900">{{ $secteur->nom_secteur }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{{ $secteur->code_secteur ?? '-' }}
                    </td>

                    <td class="px-6 py-4 whitespace-nowrap text-center">
                        @if($secteur->geom_secteur)
                ...
                <tr>
                    <td colspan="5" class="px-6 py-8 text-center text-slate-500">
                        Aucun secteur n'a encore été créé.
                    </td>
                </tr>
                @endforelse
            </tbody>
