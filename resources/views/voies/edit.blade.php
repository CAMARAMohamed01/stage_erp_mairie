...
@section('header_title', 'Modifier : ' . $voie->nom_voie)

@section('content')
    <div class="max-w-5xl mx-auto">
        <form action="{{ route('voies.update', $voie->id_voie) }}" method="POST"
            ...
            <div class="flex items-center gap-3 border-b border-slate-100 pb-4">
                ...
                <div>
                    <h2 class="text-xl font-bold text-slate-900">Mise à jour de la Voie</h2>
                    <p class="text-sm text-slate-500">Édition de l'ensemble des métadonnées géographiques et réglementaires.
                    </p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="nom_voie" required maxlength="100"
                            value="{{ old('nom_voie', $voie->nom_voie) }}"
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <div>
                            ...
                            <input type="text" name="numero_voie" maxlength="10"
                                value="{{ old('numero_voie', $voie->numero_voie) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="ancien_numero" maxlength="20"
                                value="{{ old('ancien_numero', $voie->ancien_numero) }}"
                                class="w-full border-slate-300 rounded-lg text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1">N° Provisoire</label>
                            <input type="text" name="num_provisoire" maxlength="20"
                                value="{{ old('num_provisoire', $voie->num_provisoire) }}"
                                class="w-full border-slate-300 rounded-lg text-sm">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="categorie_voie" maxlength="80"
                                value="{{ old('categorie_voie', $voie->categorie_voie) }}"
                                class="w-full border-slate-300 rounded-lg">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-1">Statut Juridique</label>
                            <input type="text" name="statut_juridique" maxlength="50"
                                value="{{ old('statut_juridique', $voie->statut_juridique) }}"
                                class="w-full border-slate-300 rounded-lg">
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-3 gap-3">
                        <div>
                            ...
                            <input type="number" name="longueur_reelle_ml"
                                value="{{ old('longueur_reelle_ml', $voie->longueur_reelle_ml) }}"
                                class="w-full border-slate-300 rounded-lg text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1">Long. Classée (ml)</label>
                            <input type="number" name="longueur_classee_ml"
                                value="{{ old('longueur_classee_ml', $voie->longueur_classee_ml) }}"
                                class="w-full border-slate-300 rounded-lg text-sm">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-700 mb-1">Larg. Moyenne (m)</label>
                            <input type="number" step="0.01" name="largeur_moyenne_m"
                                value="{{ old('largeur_moyenne_m', $voie->largeur_moyenne_m) }}"
                                class="w-full border-slate-300 rounded-lg text-sm">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="point_origine" maxlength="50"
                                value="{{ old('point_origine', $voie->point_origine) }}"
                                class="w-full border-slate-300 rounded-lg text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-1">Point Extrémité</label>
                            <input type="text" name="point_extremite" maxlength="100"
                                value="{{ old('point_extremite', $voie->point_extremite) }}"
                                class="w-full border-slate-300 rounded-lg text-sm">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="definition_trace" rows="1"
                            class="w-full border-slate-300 rounded-lg text-sm">{{ old('definition_trace', $voie->definition_trace) }}</textarea>
                    </div>
                </div>
                ...
                <div class="space-y-4 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="conformite_cadastrale" maxlength="100"
                            value="{{ old('conformite_cadastrale', $voie->conformite_cadastrale) }}"
                            class="w-full border-slate-300 rounded-lg">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="observations_statut" rows="2"
                            class="w-full border-slate-300 rounded-lg text-sm">{{ old('observations_statut', $voie->observations_statut) }}</textarea>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="historique_incorporation" rows="2"
                            class="w-full border-slate-300 rounded-lg text-sm">{{ old('historique_incorporation', $voie->historique_incorporation) }}</textarea>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
                        <div class="pt-2">
                            <label
                                ...
                                <input type="checkbox" name="est_pdipr" value="1" {{ old('est_pdipr', $voie->est_pdipr) ? 'checked' : '' }}
                                ...
                                <div>
                                    ...
                                    <span class="text-[10px] text-slate-500 block">Plan Départemental</span>
                                </div>
                            </label>
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-bold text-slate-700 mb-1">Intérêt Touristique /
                                Remarques</label>
                            <textarea name="interet_touristique" rows="2"
                                class="w-full border-slate-300 rounded-lg text-sm">{{ old('interet_touristique', $voie->interet_touristique) }}</textarea>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex justify-end gap-3 pt-6 border-t border-slate-100">
                ...
                <button type="submit"
                    class="px-5 py-2.5 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition">✅ Confirmer
