...
@section('title', 'Modifier le Signalement #' . $signalement->id_sig)
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-lg overflow-hidden">
        <div class="bg-amber-500 px-8 py-6">
            <h1 class="text-2xl font-bold text-white">Modification du signalement #{{ $signalement->id_sig }}</h1>
        </div>

        <form action="{{ route('signalements.update', $signalement->id_sig) }}" method="POST" class="p-8 space-y-8">
            @csrf
            @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="id_tiers" id="id_tiers"
                            ...
                            <option value="{{ $t->id_tiers }}"
                                {{ $signalement->id_tiers == $t->id_tiers ? 'selected' : '' }}>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="emetteur_nom" id="emetteur_nom"
                                value="{{ $signalement->emetteur_nom }}"
                                class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-amber-500">
                        </div>
                        <div>
                            ...
                            <input type="text" name="emetteur_contact" id="emetteur_contact"
                                value="{{ $signalement->emetteur_contact }}"
                                class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-amber-500">
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="mode_reception" required
                            ...
                            <option value="Téléphone"
                                {{ $signalement->mode_reception == 'Téléphone' ? 'selected' : '' }}>📞 Téléphone
                            </option>
                            <option value="Email" {{ $signalement->mode_reception == 'Email' ? 'selected' : '' }}>📧
                                Email</option>
                            <option value="Accueil" {{ $signalement->mode_reception == 'Accueil' ? 'selected' : '' }}>🏢
                                Accueil Mairie</option>
                            <option value="Application"
                                {{ $signalement->mode_reception == 'Application' ? 'selected' : '' }}>📱 Application
                                Mobile</option>
                            <option value="Courrier" {{ $signalement->mode_reception == 'Courrier' ? 'selected' : '' }}>
                                ✉️ Courrier</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="id_cat" required
                            ...
                            <option value="{{ $cat->id_cat }}"
                                {{ $signalement->id_cat == $cat->id_cat ? 'selected' : '' }}>{{ $cat->libelle }}
                            </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" required
                        class="w-full border-slate-300 rounded-lg shadow-sm focus:ring-amber-500">{{ $signalement->description }}</textarea>
                </div>
            </div>
