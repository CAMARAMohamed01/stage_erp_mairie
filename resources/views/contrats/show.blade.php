            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden mt-6">
                ...
                <div class="divide-y divide-slate-100 bg-white">
                    ...
                    <div class="p-4 flex flex-wrap justify-between items-center text-sm hover:bg-slate-50">
                        <div>
                            <p class="font-semibold text-slate-800">{{ $loc->nom_equipement }}
                                (x{{ $loc->quantite_louee }})</p>
                            <p class="text-xs text-slate-400">
                                Réf Décision : <span
                                    class="font-mono text-slate-600 font-bold">{{ $loc->numero_decision }}</span>
                                | État départ : <span
                        ...
                        <div class="text-right text-xs">
                            <span
                                class="px-2 py-0.5 font-bold rounded bg-blue-50 text-blue-700 border border-blue-100">{{ $loc->statut_ligne }}</span>
                            <p class="text-[10px] text-slate-400 mt-1">Du
                                {{ \Carbon\Carbon::parse($loc->date_debut_utilisation)->format('d/m/Y') }} au
                                {{ $loc->date_fin_utilisation ? \Carbon\Carbon::parse($loc->date_fin_utilisation)->format('d/m/Y') : 'Indéterminé' }}
                            </p>
                        </div>
                    </div>
                    @empty
                    <p class="p-6 text-center text-sm text-slate-400 italic">Aucun équipement n'est actuellement
                        rattaché à ce contrat de location.</p>
                    @endforelse
                </div>
