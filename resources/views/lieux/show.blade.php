...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('lieux.edit', $lieu->id_lieu) }}" class=" px-4 py-2 bg-slate-900 text-white text-sm font-semibold rounded-lg hover:bg-slate-800
                                transition">
                Modifier
            </a>
            @endcan
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="p-4 bg-slate-50 border-b border-slate-200 font-bold text-sm text-slate-800">🌍 Localisation de
            l'Espace Public</div>
        <div id="map" data-lieu="{{ $lieu->geojson_lieu ?? '' }}" data-parcelle=" {{ $lieu->geojson_parcelle ?? '' }}"
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // 🚨 L'erreur était sur cette ligne (L.tileLayer)
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        maxZoom: 19,
        attribution: '© OpenStreetMap'
    }).addTo(map);

    // Force le redessin de la carte pour éviter le bug de l'écran gris
    setTimeout(function() {
        map.invalidateSize();
    }, 200);
    ...
    // 1. Dessiner la Parcelle (Polygone vert)
        ...
        try {
            ...
            var markerLayer = L.geoJSON(lieuData, {
                pointToLayer: function(feature, latlng) {
                    pointTarget = latlng; // On mémorise les coordonnées exactes du marqueur
    ...
    // 3. Centrage de la caméra
    if (pointTarget) {
        // S'il y a un point GPS, on centre dessus
