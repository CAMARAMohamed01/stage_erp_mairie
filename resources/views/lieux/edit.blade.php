            ...
            <div class="space-y-4">
                <div>
                    ...
                    <select name="id_parcelle" required class="w-full rounded-lg border-slate-300 text-sm bg-slate-50">
                        ...
                        <option value="{{ $p->id_parcelle }}"
                            {{ $lieu->id_parcelle == $p->id_parcelle ? 'selected' : '' }}>Section
                    ...
                    <div>
                        ...
                        <select name="id_batiment" class="w-full rounded-lg border-slate-300 text-sm">
                            ...
                            <option value="{{ $bat->id_batiment }}"
                                {{ $lieu->id_batiment == $bat->id_batiment ? 'selected' : '' }}>{{ $bat->nom_bat }}
                            </option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="id_type_erp" class="w-full rounded-lg border-slate-300 text-sm">
                            ...
                            <option value="{{ $erp->id_type_erp }}"
                                {{ $lieu->id_type_erp == $erp->id_type_erp ? 'selected' : '' }}>Cat.
                ...
                <div class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <input type="hidden" name="geojson_data" id="geojson_data" value="{{ $lieu->geojson_lieu ?? '' }}">
                ...
                <div>
                    ...
                    <select name="id_decision_reglement"
                        ...
                        <option value="{{ $dec->id_decision }}"
                            {{ $lieu->id_decision_reglement == $dec->id_decision ? 'selected' : '' }}>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // 1. Initialisation de la carte (Zoom par défaut sur la commune)
    ...
    // On s'assure que la carte recalcule bien sa taille (évite l'effet carte blanche)
    setTimeout(function() {
        map.invalidateSize();
    }, 200);
    ...
    var parcelleBounds = null;
    var pointTarget = null;

    // 2. Dessiner la Parcelle (Polygone vert)
    if (geojsonParcelleStr && geojsonParcelleStr.trim() !== '') {
        try {
            ...
            if (parcelleLayer.getBounds && parcelleLayer.getBounds().isValid()) {
                parcelleBounds = parcelleLayer.getBounds();
            }
        } catch (e) {
            console.error("Erreur de rendu de la parcelle :", e);
        }
    }

    // 3. Dessiner le Lieu Public (Marqueur GPS)
    if (geojsonLieuStr && geojsonLieuStr.trim() !== '') {
        try {
            var lieuData = JSON.parse(geojsonLieuStr);
            var markerLayer = L.geoJSON(lieuData, {
                pointToLayer: function(feature, latlng) {
                    pointTarget = latlng; // On sauvegarde les coordonnées exactes du point
                    return L.marker(latlng).bindPopup('<b>{{ $lieu->nom_lieu }}</b>');
                }
            }).addTo(map);
        } catch (e) {
    ...
    // 4. Centrage intelligent de la caméra
    if (pointTarget) {
        // S'il y a un point GPS, on centre direct dessus sans utiliser fitBounds (anti-crash)
        map.setView(pointTarget, 18);
    } else if (parcelleBounds) {
        // Sinon, on centre sur la parcelle
        map.fitBounds(parcelleBounds, {
            padding: [30, 30]
        });
    }
