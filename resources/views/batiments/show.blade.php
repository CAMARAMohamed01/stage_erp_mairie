...
<div class="max-w-7xl mx-auto space-y-6 pb-12">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-6">
            ...
            @php
            $actionsActives = $actions->filter(function($action) use ($batiment, $locaux) {
            $statut = strtolower(trim($action->statut_action ?? ''));

            // Exclusion des statuts de clôture ou d'abandon
            $estActif = $statut !== 'terminé' && $statut !== 'terminée' && $statut !== 'abandonné' && $statut !==
            'abandonnée';

            // L'action est-elle liée directement au bâtiment ?
            $estDuBatiment = ($action->id_batiment == $batiment->id_batiment);

            // Tolérance : Si l'action n'est pas sur le bâtiment lui-même, l'est-elle sur un local de CE bâtiment ?
            if (!$estDuBatiment && !empty($action->id_local) && isset($locaux)) {
            $estDuBatiment = $locaux->contains('id_local', $action->id_local);
            }

            return $estActif && $estDuBatiment;
            });
            @endphp
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div class="p-4 bg-red-50/50 border-b border-red-100">
                    <h3 class="text-sm font-bold text-red-800 tracking-tight flex items-center gap-2">🚨 Actions
                        Actives ({{ $actionsActives->count() }})</h3>
                </div>
                <div class="p-4 space-y-3 max-h-52 overflow-y-auto">
                    @forelse($actionsActives as $sig)
