...
@section('content')
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div
            class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm flex items-center justify-between hover:shadow-md transition">
            <div>
                <div class="text-slate-500 text-sm font-bold uppercase tracking-wider mb-1">Chantiers</div>
                <div class="text-3xl font-extrabold text-slate-800">{{ count($interventionsEnCours) }}<span
                        class="text-sm font-medium text-slate-400 ml-1">en cours</span></div>
            </div>
        ...
        <div
            class="bg-white rounded-xl border border-slate-200 p-6 shadow-sm flex items-center justify-between hover:shadow-md transition">
            <div>
                <div class="text-slate-500 text-sm font-bold uppercase tracking-wider mb-1">Équipements</div>
                <div class="text-3xl font-extrabold text-red-600">{{ count($equipementsEnPanne) }}<span
                        class="text-sm font-medium text-slate-400 ml-1">HS</span></div>
            </div>
        ...
        <div
            ...
            <div>
                ...
                <div class="text-3xl font-extrabold text-slate-800">{{ count($controles) }}<span
                        class="text-sm font-medium text-slate-400 ml-1">à venir</span></div>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            ...
            <div class="flex-1 p-0">
                <ul class="divide-y divide-slate-100 text-sm">
                    ...
                    @empty
                        <li class="px-6 py-8 text-slate-400 text-center flex flex-col items-center">
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            <div class="bg-blue-50 px-6 py-4 border-b border-blue-100 flex justify-between items-center">
                <h2 class="font-bold text-blue-900 text-lg flex items-center">
            ...
            <div class="flex-1 p-0">
                <ul class="divide-y divide-slate-100 text-sm">
                    @forelse($interventionsEnCours as $int)
                        <li class="hover:bg-blue-50/50 transition-colors">
                            <a href="{{ route('interventions.show', $int->id_int) }}" class="block px-6 py-4">
                                <div class="flex justify-between items-start mb-1">
                                    <span class="font-bold text-slate-800">{{ $int->type_intervention }}</span>
                                    <span
                                        class="text-xs font-semibold bg-blue-100 text-blue-700 px-2 py-0.5 rounded">#{{ $int->id_int }}</span>
                                </div>
                                <div class="flex justify-between items-center mt-2 text-slate-500 text-xs">
                                    <span>Ouverte le {{ \Carbon\Carbon::parse($int->date_ouverture)->format('d/m/Y') }}</span>
                                    <span class="text-orange-500 font-medium">{{ $int->statut_global }}</span>
                                </div>
                            </a>
                        </li>
                    @empty
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="bg-red-50 px-6 py-4 border-b border-red-100 flex justify-between items-center">
                <h2 class="font-bold text-red-900 text-lg flex items-center">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($equipementsEnPanne as $equipement)
                            <tr class="hover:bg-slate-50 transition-colors cursor-pointer"
                                ...
                                <td class="px-6 py-4 text-slate-500 text-right">{{ $equipement->marque ?? 'Réf. N/A' }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="2" class="px-6 py-6 text-slate-400 text-center italic">Tous les équipements sont
                                    fonctionnels.</td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div
            class="bg-gradient-to-r from-slate-900 to-slate-800 text-white p-6 rounded-xl border border-slate-700 shadow-lg my-6 flex items-center justify-between">
            <div>
                <h3 class="text-lg font-bold tracking-tight">Moteur de Planification Préventive</h3>
                <p class="text-sm text-slate-400 mt-1">Analyse les fréquences des contrôles réglementaires pour automatiser
                    la création des interventions.</p>
            </div>
            <form action="{{ route('admin.preventif.generer') }}" method="POST">
                @csrf
                <button type="submit"
                    class="px-5 py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-semibold text-sm rounded-lg shadow-md transition duration-150 flex items-center gap-2">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($controles as $controle)
                            <tr class="hover:bg-slate-50 transition-colors">
                                <td class="px-6 py-4 font-bold text-slate-700">{{ $controle->designation }}</td>
                                <td class="px-6 py-4 text-right">
                                    <span class="bg-slate-100 text-slate-600 px-2 py-1 rounded text-xs font-semibold">Tous les
                                        {{ $controle->frequence_mois }} mois</span>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="2" class="px-6 py-6 text-slate-400 text-center italic">Aucune échéance paramétrée.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
