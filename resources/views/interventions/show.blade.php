    ...
    <div class="max-w-5xl mx-auto">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div class="p-4 flex justify-between items-center text-sm">
                                ...
                                @can('check-permission', ['Interventions', 'ecriture'])
                                    <form action="{{ route('interventions.materiel.destroy', $mat->id_achat) }}" method="POST"
                                        onsubmit="return confirm('Supprimer cette ligne de matériel ?');">
                                        @csrf @method('DELETE')
                                        <button type="submit"
                                            class="text-red-500 hover:text-red-700 font-bold px-2 py-1 bg-red-50 rounded text-xs">
                                            ❌
                                        </button>
                                    </form>
                                @endcan
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
                    <h3 class="font-bold text-slate-900 mb-4 border-b pb-2">Localisation de l'intervention</h3>
                    <div class="space-y-4">

                        {{-- Bâtiment --}}
                        <div>
                            <span class="text-xs font-bold text-slate-400 uppercase tracking-wider block">🏛️ Bâtiment
                                communal</span>
                            @if($intervention->batiment)
                                <p class="text-sm font-semibold text-slate-800 mt-1">{{ $intervention->batiment->nom_bat }}</p>
                        ...
                        <div class="pt-3 border-t border-slate-100">
                            ...
                            @if($intervention->local)
                                <p class="text-sm font-semibold text-slate-800 mt-1">{{ $intervention->local->nom_local }}</p>
                                <span class="text-xs text-slate-500">Niveau : {{ $intervention->local->niveau ?? 'RDC' }}</span>
                            @else
                                <p class="text-xs text-slate-400 italic mt-1">Aucun local intérieur rattaché.</p>
                            @endif
                        </div>

                        {{-- Espace Public --}}
                        <div class="pt-3 border-t border-slate-100">
                            <span class="text-xs font-bold text-slate-400 uppercase tracking-wider block">🌳 Espace Public
                                associé</span>
                            @if($intervention->lieuPublic)
                                <div class="bg-slate-50 p-2 rounded border border-slate-100 text-sm mt-2">
                                    <p class="font-bold text-slate-800">{{ $intervention->lieuPublic->nom_lieu }}</p>
                                    <span class="text-xs text-slate-500">Typologie :
                                        {{ $intervention->lieuPublic->typologie_lieu ?? 'Espace public' }}</span>
                                </div>
                            @else
                                <p class="text-xs text-slate-400 italic mt-1">Aucun espace public extérieur lié.</p>
                            @endif
                        </div>
